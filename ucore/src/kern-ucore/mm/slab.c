...
static void
cache_estimate(size_t order, size_t objsize, size_t align, bool off_slab,
	       size_t * remainder, size_t * num)
{
	...
	if (off_slab) {
		...
		nr_objs =
		    (slab_size - sizeof(slab_t)) / (objsize +
						    sizeof(kmem_bufctl_t));
		while (slab_mgmt_size(nr_objs, align) + nr_objs * objsize >
		       slab_size) {
			nr_objs--;
		}
		if (nr_objs > SLAB_LIMIT) {
...
static void
calculate_slab_order(kmem_cache_t * cachep, size_t objsize, size_t align,
		     bool off_slab, size_t * left_over)
{
	size_t order;
	for (order = 0; order <= KMALLOC_MAX_ORDER; order++) {
		size_t num, remainder;
		cache_estimate(order, objsize, align, off_slab, &remainder,
			       &num);
		if (num != 0) {
			if (off_slab) {
				size_t off_slab_limit =
...
static void init_kmem_cache(kmem_cache_t * cachep, size_t objsize, size_t align)
{
	...
	size_t left_over;
	calculate_slab_order(cachep, objsize, align, cachep->off_slab,
			     &left_over);
	...
	if (cachep->off_slab) {
		cachep->offset = 0;
		cachep->slab_cachep =
		    slab_cache + (getorder(mgmt_size) - MIN_SIZE_ORDER);
	} else {
...
void check_slab(void)
{
	...
	cachep0 = slab_cache;
	assert(cachep0->objsize == 32 && cachep0->num > 1
	       && !cachep0->off_slab);
	...
	assert(slabp0->inuse == 1
	       && list_next(&(slabp0->slab_link)) == &(cachep0->slabs_notfull));
	...
	for (i = 0; i < cachep0->page_order; i++, p1 = NEXT_PAGE(p1)) {
		assert(PageSlab(p1));
		assert(GET_PAGE_CACHE(p1) == cachep0
		       && GET_PAGE_SLAB(p1) == slabp0);
	}
	...
	if (cachep0->num == 1) {
		...
		assert(!list_empty(&(cachep0->slabs_notfull)));
		slabp0 =
		    le2slab(list_next(&(cachep0->slabs_notfull)), slab_link);
	}
	...
	if (cachep1->num == 1) {
		...
		assert(!list_empty(&(cachep1->slabs_notfull)));
		slabp1 =
		    le2slab(list_next(&(cachep1->slabs_notfull)), slab_link);
	}
	...
	for (i = 0; i < order_size; i++, p0 = NEXT_PAGE(p0)) {
		assert(PageSlab(p0));
		assert(GET_PAGE_CACHE(p0) == cachep0
		       && GET_PAGE_SLAB(p0) == slabp0);
	}
